...
'@
    ...
    Foreach ($Volume in $Volumes) {
        ...
        [void]$DiskArray.Add("$VolLetter Avg Read (Bytes/sec) : $([math]::Round(($DiskCounterArray | Where-Object { $_.Name -eq 'Read Bytes' } | Select-Object -ExpandProperty 'Avg'), 2))")
        [void]$DiskArray.Add("$VolLetter Max Read (Bytes/sec) : $([math]::Round(($DiskCounterArray | Where-Object { $_.Name -eq 'Read Bytes' } | Select-Object -ExpandProperty 'Max'), 2))")
        [void]$DiskArray.Add("$VolLetter Avg Write (Bytes/sec) : $([math]::Round(($DiskCounterArray | Where-Object { $_.Name -eq 'Write Bytes' } | Select-Object -ExpandProperty 'Avg'), 2))")
        [void]$DiskArray.Add("$VolLetter Max Write (Bytes/sec) : $([math]::Round(($DiskCounterArray | Where-Object { $_.Name -eq 'Write Bytes' } | Select-Object -ExpandProperty 'Max'), 2))")
